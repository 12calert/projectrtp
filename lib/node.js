...
const channelmap = {
  ...
  /* Live media-flow counters (see livestats() in the Rust facade) - a query,
     so unlike the fire-and-forget ops above it replies with
     { action: "livestats", livestats: {...} } for the server-side proxy to
     resolve its pending livestats() promise with. */
  "livestats": ( chan, msg, conn ) => {
    /* echo reqid so the server matches the reply to its request */
    conn.send( { "id": chan.id, "uuid": chan.uuid, "action": "livestats", "reqid": msg.reqid, "livestats": chan.livestats() } )
  },
...
class rtpnodeconnection {
  ...
  _updatechannel( msg ) {
    ...
    if( msg.channel in channelmap ) {
      channelmap[ msg.channel ]( chan, msg, this )
    } else {
  ...
  send( msg, cb = undefined ) {
    ...
    this.parent._post( msg, ( modifiedmsg ) => {
      ...
      delete modifiedmsg.playrecord
      delete modifiedmsg.stopplay
  ...
  async _openchannel( msg ) {
    ...
    // Guard against an open that never completes. Previously this crashed the
    // whole node (process.exit(1)), which took every active call on this node
    // with it — one stuck open could topple the entire media server. Instead
    // reply with an error and account for the channel we reserved, so the
    // node stays up for the rest of its channels.
    let timedout = false
    const timerid = setTimeout( () => {
      if( timedout ) return
      timedout = true
      console.error( "Timeout opening channel", msg )
      const timeoutsend = { error: "open failed: timeout", msg }
      this.send( timeoutsend, () => this.#cleanup() )
    }, 4000 )
    ...
    try {
      chan = await this.parent.prtp.openchannel( msg, ( cookie ) => {
        ...
        // A late callback after the timeout must not operate on torn-down
        // state or send stale responses — the open already failed.
        if( timedout ) {
          if( chan ) {
            channels.delete( chan.uuid )
            try { chan.close( "open timeout" ) } catch( e ) {}
          }
          return
        }
      ...
      console.error( "Error opening channel:", e.message || e )
      const opentosend = { error: "open failed: " + ( e.message || e ), msg }
    ...
    if( !chan ) {
      clearTimeout( timerid )
      console.error( "Error opening channel - returned null", msg )
      const opentosend = { error: "open failed: returned null", msg }
      await this.send( opentosend )
      this.#cleanup()
      return false
    }

    // The open resolved after the timeout already fired — the server has been
    // told this open failed, so discard the channel rather than half-register
    // it and emit a late/duplicate "open" event.
    if( timedout ) {
      try { chan.close( "open timeout" ) } catch( e ) {}
      this.#cleanup()
      return false
  ...
  listen() {
    ...
    this.server = net.createServer( ( connection ) => {
      try {
        ...
        this.connections.set( con.connectionid, con )
        connection.on( "close", () => {
          this.connections.delete( con.connectionid )
        } )
      } catch( e ) {
