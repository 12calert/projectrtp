...
class channel {
  ...
  /* livestats() requests awaiting a reply: reqid -> resolve, in request order */
  #livestatspending = new Map()
  #livestatsreqid = 0
 ...
 */
  ...
  /**
   * @summary Live media-flow counters for this channel, fetched from the node
   * that hosts it: { relay, in: { count, accepted, rtcp, decryptfailed, prekey },
   * out: { count, dropped, ptdropped }, streams: [ { source, ssrc, sourcessrc, pt } ] }.
   * The "registered but no media flowing" detector for relay (video) legs -
   * poll it and watch in.accepted + in.rtcp. `streams` is a relay leg's
   * outbound stream mapping (see the channel typedef in index.js). A local
   * channel returns the same object synchronously, so always await it.
   * Polling traffic is kept out of the channel history and the "all" callback.
   * @param { number } [ timeout ] - mS to wait for the node's reply
   * @returns { Promise< object > }
   */
  livestats( timeout = 2000 ) {
    return new Promise( ( resolve, reject ) => {
      /* each request carries its own id, echoed in the reply, so concurrent
         callers each get their own answer (and a late reply to a request
         that timed out is not handed to the next caller) */
      const reqid = ++this.#livestatsreqid
      const timer = setTimeout( () => {
        this.#livestatspending.delete( reqid )
        reject( new Error( "timed out waiting for livestats" ) )
      }, timeout )
      this.#livestatspending.set( reqid, ( stats ) => {
        clearTimeout( timer )
        resolve( stats )
      } )
      this._write( { "channel": "livestats", reqid }, false )
    } )
  }

  /**
   * Resolve the livestats() request a reply answers: by its echoed reqid,
   * or - from a node that predates reqid - the oldest outstanding one, as
   * replies arrive in request order over the one node connection.
   * @param { object } msg
   */
  #onlivestats( msg ) {
    let reqid = msg.reqid
    if( undefined === reqid ) reqid = this.#livestatspending.keys().next().value
    const done = this.#livestatspending.get( reqid )
    if( !done ) return
    this.#livestatspending.delete( reqid )
    done( msg.livestats )
  }
  ...
  */
  ...
  @param { string } [ options.direction="both" ] - which side(s) of the call to record: "in" (only what the
    remote party sends us — use for STT captures so our own playback never lands in the file), "out" (only
    what we send: player/echo, or the far leg in a mix), "both" (default - mono sums both legs, stereo
    interleaves L=in R=out)
  ...
  */
  ...
  /**
  @summary Combined play then record - starts playback then immediately activates recording when play ends (zero-gap).
  Optionally supports barge-in where incoming speech interrupts playback and starts recording with pre-buffered audio.
  @param { Object } options
  @param { soundsoup } options.soup - sound soup definition for playback
  @param { Object } options.record - record options (same as record() options)
  @param { boolean } [ options.interrupt=false ] - enable barge-in via power detection
  @param { number } [ options.bargeinpower ] - power threshold for barge-in
  @param { number } [ options.bargeinpoweraveragepackets=5 ] - power averaging window for barge-in
  @param { boolean } [ options.concurrent=false ] - concurrent-capture mode: the recorder opens immediately and
    records the caller's answer *while the prompt keeps playing* (gated by startabovepower until they speak); the
    prompt is NOT interrupted by inbound energy. Call stopplay() to end the prompt once the answer is acceptable.
  ...
  */
  ...
  /**
  @summary Stop the currently-playing prompt immediately, leaving any running recorder untouched.
  Companion to playrecord({ concurrent: true }): capture the answer while the prompt plays, then call this
  to "fire" the interrupt once the answer is acceptable.
  @returns { boolean }
  */
  stopplay() {
    this._write( {
  ...
  */
  ...
  _on( msg ) {
    ...
    /* a reply to livestats() - not a channel event, so it neither lands in
       the history nor reaches the "all" callback */
    if( "livestats" === msg.action ) {
      this.#onlivestats( msg )
      return
    }
   ...
   * @private
   * @param { object } msg 
   * @param { boolean } [ record ] - add to the channel history
   * @returns { boolean }
   */
  _write( msg, record = true ) {
    msg.id = this.id
    ...
    msg.now = new Date()
    msg.timestamp = msg.now.getTime()
    if( record ) this.history.push( msg )
