...
function randomenode() {
  ...
  if ( nodes.size > 0) {
    for( let node of nodes.values() ) {
      if ( node.status.size !== 0 && !node.rejectconnections ) {
        let loadPerCPU = node.status.channel.current /  node.status.workercount
        if ( loadPerCPU < min ) {
          min = loadPerCPU
          min_node = node
        }
    ...
    if ( listeningnodes.length ) {
      let index = between( 0, listeningnodes.length )
      for( let node of listeningnodes.length ) {
        if( 0 === index ) {
          return node
        }
        index--
